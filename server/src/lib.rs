...
//!
//! Exposé en bibliothèque pour que le runner (v0.3) construise **exactement**
//! la même observation JSON que celle servie aux agents par HTTP.
//!
...
impl ActionDto {
    pub fn to_action(&self) -> Result<Action, ErrorCode> {
        Ok(match self {
            ActionDto::Build {
                building_type,
                x,
...
/// L'observation JSON exacte que reçoit un agent par HTTP — le runner s'en sert
/// pour que les agents in-process voient le même état que par le réseau.
pub fn observation(game_id: &str, g: &Game, mode: Mode) -> Value {
    json!(state_dto(game_id, g, mode))
}

fn ok(game_id: &str, g: &Game, mode: Mode) -> Reply {
    (
        StatusCode::OK,
        Json(json!({"ok": true, "state": observation(game_id, g, mode)})),
    )
}
